	...
	didInsertElement: ->
		...
		@$('.table').dataTable({
			...
			'aoColumns': @get('columns')
			 ...
			 search_input = dataTables.closest('.dataTables_wrapper').
                        find('div[id$=_filter] input')
			 search_input.addClass('form-control input-sm')
			 search_input.attr('placeholder', 'Search')

			 # Label Text: Clear search label text
			 search_label = dataTables.closest('.dataTables_wrapper').
                        find('div[id$=_filter] label')
			 ...
			 dataTablesSize_select = dataTables.closest('.dataTables_wrapper').
                                  find('div[id$=_length] select')
			 dataTablesSize_select.addClass('form-control input-sm')

			 # Label Text: Clear all the label text concerning dataTables-size
			 dataTablesSize_label = dataTables.closest('.dataTables_wrapper').
                                find('div[id$=_length] label')
